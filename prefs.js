...
var SettingsWidget = class SettingsWidget {
  ...
  constructor(prefsWindow, selectedPage = 0, selectedWorkspace = 0) {
    ...
    topbarFollowFocus.state = this._settings.get_boolean("topbar-follow-focus");
    ...
    const defaultBackgroundSwitch = this.builder.get_object(
      "use-default-background"
    );
    defaultBackgroundSwitch.state = this._settings.get_boolean(
      "use-default-background"
    );
    defaultBackgroundSwitch.connect("state-set", (obj, state) => {
      this._settings.set_boolean("use-default-background", state);
    });
    const backgroundPanelButton = this.builder.get_object(
    ...
    let wsIndicesSelectedFirst = swapArrayElements(
      wsIndices.slice(),
      0,
      selectedWorkspace
    );
  ...
  createWorkspacePage(settings, index) {
    ...
    let hideTopBarSwitch = new Gtk.Switch({
      active: !settings.get_boolean("show-top-bar"),
    });
    ...
    if (color === "") color = palette[index % palette.length];
  ...
  getWorkspaceName(settings, index) {
    ...
    if (name === "") name = this.workspaceNames[index];
...
function createKeybindingWidget(settings, searchEntry) {
  ...
  filteredModel.set_visible_func((model, iter) => {
  ...
  descriptionColumn.add_attribute(
    descriptionRenderer,
    "text",
    COLUMN_DESCRIPTION
  );
  ...
  accelRenderer.connect("accel-cleared", (accelRenderer, path) => {
    let iter = getOk(filteredModel.get_iter_from_string(path));
    if (!iter) return;
    ...
    if (!model.iter_next(nextSibling)) nextSibling = null;
...
function annotateKeybindings(model, settings) {
  ...
  model.foreach((model, path, iter) => {
    ...
    let accels = settings.get_strv(id);
    let index = model.get_value(iter, COLUMN_INDEX);
    if (index === -1 || accels.length === 0) return true;
...
function createFileChooserButton(
  settings,
  key,
  iconName,
  symbolicIconName,
  ...
  if (symbolicIconName) {
    buttonBox.append(
      new Gtk.Image({ icon_name: symbolicIconName, margin_start: 8 })
    );
  }
  ...
  syncStringSetting(settings, key, (path) => {
    ...
    buttonLabel.label =
      path == "" ? "(None)" : GLib.filename_display_basename(path);
  });
