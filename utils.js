...
function toggleCloneMarks() {
  ...
  function markCloneOf(metaWindow) {
    if (metaWindow.clone) {
      ...
      metaWindow.clone.background_color =
        imports.gi.Clutter.color_from_string("red")[1];
    }
  }
...
function infoMetaWindow(metaWindow) {
  ...
  let info = [`(win: ${id}) ${trace}`, `Title: ${metaWindow.title}`];
...
function shortTrace(skip = 0) {
  ...
  trace = trace
    .filter(([f, ln]) => f !== "dynamic_function_ref")
    .map(([f, ln]) => (f === "" ? "?" : f + ":" + ln));
