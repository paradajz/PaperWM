...
function useNeigbour(dir, action) {
  return (metaWindow) => {
    ...
    if (!space[i + dir]) return action(undefined);
...
function findNonVisibleIndex(space, metaWindow, dir = 1, margin = 1) {
  ...
  while (
    0 <= k &&
    k < space.length &&
    space.isFullyVisible(space[k][0], margin)
  ) {
    k += dir;
  }
  return k;
...
function fitAvailable(metaWindow) {
  ...
  let rightEdge =
    space.targetX + rightMost.clone.targetX + rightMost.clone.width;
  ...
  let available =
    ...
    availableRight +
    availableLeft -
    Tiling.prefs.horizontal_margin * 2;
  ...
  if (a + 1 === b - 1) {
    ...
    Tiling.move_to(space, space[a + 1][0], {
      x: Tiling.prefs.horizontal_margin,
    });
  }
}
...
function cycleLayoutDirection(dir) {
  ...
  return (metaWindow, space, { navigator } = {}) => {
    ...
    if (dir === 1) moveToViewport(space, tiling, k, snapLeft);
    else moveToViewport(space, tiling, j, snapLeft);
  };
}
...
function cycleLayouts(binding = "<Super>d") {
  ...
  Keybindings.bindkey(binding, "cycle-layouts", action, {
    opensNavigator: true,
  });
}
...
function tileInto(dir = -1) {
  return (metaWindow, space) => {
    ...
    let jTo = jFrom + dir;
    if (jTo < 0 || jTo >= space.length) return;
...
function bindTileInto(
  leftBinding = "<Super><Alt>Left",
  rightBinding = "<Super><Alt>Right"
) {
  let options = { activeInNavigator: true };
  if (leftBinding)
    Keybindings.bindkey(
      leftBinding,
      "tile-into-left-column",
      tileInto(-1),
      options
    );
  if (rightBinding)
...
function bindTileOut(left = "<Super><Ctrl>k", right = "<Super><Ctrl>l") {
  Keybindings.bindkey(left, "tile-out-left", tileOut(-1), {
    activeInNavigator: true,
  });
  Keybindings.bindkey(right, "tile-out-right", tileOut(1), {
    activeInNavigator: true,
  });
}

function bindFitAvailable(
  left = "<Super>j",
  focus = "<Super>k",
  right = "<Super>l"
) {
  left &&
    Keybindings.bindkey(
      left,
      "fit-available-width-left",
      useNeigbour(-1, fitAvailable),
      { activeInNavigator: true }
    );
  focus &&
    Keybindings.bindkey(focus, "fit-available-width", fitAvailable, {
      activeInNavigator: true,
    });
  right &&
    Keybindings.bindkey(
      right,
      "fit-available-width-right",
      useNeigbour(1, fitAvailable),
      { activeInNavigator: true }
    );
}

function bindCycleLayoutDirection(
  left = "<Super><Shift>d",
  right = "<Super>d"
) {
  Keybindings.bindkey(left, "cycle-layout-left", cycleLayoutDirection(-1), {
    opensNavigator: true,
  });
  Keybindings.bindkey(right, "cycle-layout-right", cycleLayoutDirection(1), {
    opensNavigator: true,
  });
}
