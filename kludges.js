...
if (!global.display.get_monitor_neighbor_index) {
  ...
  global.display.constructor.prototype.get_monitor_neighbor_index = function (
    ...args
  ) {
...
if (!global.display.set_cursor) {
  global.display.constructor.prototype.set_cursor =
    global.screen.set_cursor.bind(global.screen);
}
...
if (!St.Settings) {
  ...
  St.Settings = {
    get: function () {
      return polyfillSettings;
    }, // ASSUMTION: no need to call get_default each time
  };
}
...
if (version[0] >= 3 && version[1] < 34) {
  ...
  function _easeActor(actor, params) {
    ...
    let animatedProps = Object.keys(params).map((p) =>
      p.replace("_", "-", "g")
    );
...
function _realRecalculateWindowPositions(flags) {
  ...
  if (space) {
    clones.sort((a, b) => {
      ...
      if (ia === -1 && ib === -1) {
        return (
          a.metaWindow.get_stable_sequence() -
    ...
    clones.sort((a, b) => {
      return (
        a.metaWindow.get_stable_sequence() - b.metaWindow.get_stable_sequence()
      );
    });
  }

  if (this._reservedSlot) clones.push(this._reservedSlot);
...
function getOriginalPosition() {
  ...
  let [x, y] = [
    space.monitor.x + space.targetX + c.targetX,
    space.monitor.y + c.y,
  ];
  return [x, y];
}
...
function init() {
  ...
  registerOverridePrototype(
    Workspace.Workspace,
    "_realRecalculateWindowPositions",
    _realRecalculateWindowPositions
  );

  registerOverridePrototype(
    Workspace.UnalignedLayoutStrategy,
    "_sortRow",
    (row) => row
  );

  registerOverridePrototype(
    WindowManager.WorkspaceTracker,
    "_checkWorkspaces",
    _checkWorkspaces
  );
  if (WindowManager.TouchpadWorkspaceSwitchAction)
    // disable 4-finger swipe
    registerOverridePrototype(
      WindowManager.TouchpadWorkspaceSwitchAction,
      "_checkActivated",
      () => false
    );
  ...
  if (Main.wm._swipeTracker)
    registerOverrideProp(
      Main.wm._swipeTracker._touchpadGesture,
      "enabled",
      false
    );
...
function enable() {
  ...
  signals.connect(
    settings,
    "changed::only-scratch-in-overview",
    scratchInOverview
  );
  signals.connect(
    settings,
    "changed::disable-scratch-in-overview",
    scratchInOverview
  );
  scratchInOverview();
...
function _checkWorkspaces() {
  ...
  if (this._pauseWorkspaceCheck || Tiling.inPreview) return true;
...
function addWindow(window, metaWindow) {
  ...
  this._windows.set(window, {
    ...
    destroyId: window.connect("destroy", () => this.removeWindow(window)),
