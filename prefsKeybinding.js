...
const actions = {
  ...
  scratch: ["toggle-scratch-layer", "toggle-scratch", "toggle-scratch-window"],
};
  ...
  class Keybinding extends GObject.Object {
    ...
    _load() {
      ...
      let combos = keystrs
        ...
        .map((keystr) => {
          if (keystr != "") return Gtk.accelerator_parse(keystr);
  ...
  class ComboRow extends Gtk.ListBoxRow {
    ...
    _onKeyPressed(controller, keyval, keycode, state) {
      ...
      this._updateKeybinding(
        new Combo({ keycode: keycode, keyval: keyvalLower, mods: modmask })
      );
